...
class video extends CI_Controller
{
    ...
    public function add_album(){
            ...
            $test=$this->db->where('album_name',$data['album_name'])->count_all_results('video_album');
             if($test>0){
                ...
                $this->global_model->insert('video_album', $data);
                $this->index();
            }
    ...
    public function delete_photo(){
        ...
        if($photo_details){
            foreach ($photo_details->result()as $row){
                $photo_name=$row->name;
                unlink(FCPATH . './assets/file/'. $photo_name);
            }
            $tables = array('photos');
